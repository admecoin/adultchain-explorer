...
const params = {
  LAST_POW_BLOCK: 10000, // 182700, // 345600
  RAMP_TO_BLOCK: 960
...
const avgBlockTime = 90; // 1 minute (adjusted blocktime)
...
const getMNSubsidy = (nHeight = 0, nMasternodeCount = 0, nMoneySupply = 0) => {
    ...
    return (getSubsidy(nHeight)) * 0.3;
  /*
  let ret = 0.0;

    if (nHeight === 1) { // initial premine 20M
        ret = blockValue;
    } else if (nHeight <= 200 && nHeight >= 2) {
        ret = blockValue;
    } else if (nHeight <= 135000 && nHeight >= 201) {
        ret = blockValue;
    } else if (nHeight <= 390000 && nHeight >= 135001) {
        ret = blockValue * 0.75;
    } else if (nHeight <= 800000 && nHeight >= 390001) {
        nSubsidy = 50;
    } else if (nHeight <= 1500000 && nHeight > 800001) {
        nSubsidy = 20;
    } else if (nHeight <= 2660026 && nHeight >= 1500001) {
        nSubsidy = 10;
    } else {
        nSubsidy = 0;
    }
...
// Data taken from: https://github.com/AdultChain/AdultChain
const getSubsidy = (nHeight = 1) => {
    let nSubsidy = 0.0;
    // let nSlowSubsidy = 50.0;

    if (nHeight === 1) {
        nSubsidy = 90000000.00; // initial premine 20M
    } else if (nHeight <= params.LAST_POW_BLOCK && nHeight >= 2) {
        nSubsidy = 0.0001;
    } else if (nHeight <= 135000 && nHeight >= params.LAST_POW_BLOCK + 1) {
        nSubsidy = 3;
    } else if (nHeight <= 390000 && nHeight >= 135001) {
        nSubsidy = 3;
    } else if (nHeight <= 800000 && nHeight >= 390001) {
